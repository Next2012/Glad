...
- **会话排序**：大厅卡片长按三秒后可拖动，长列表靠近边缘时自动滚动；平铺视图可拖动标题栏调整当前页。两个视图共用顺序，并作为当前浏览器的布局偏好保存。
- **响应式工作区**：宽屏可使用可调宽度的会话侧边栏或实时多会话平铺看板，手机使用大厅与对话双页面，并支持亮色、暗色主题。
- **图表与公式**：Codex/Claude 对话、恢复的历史消息和平铺预览均支持 Mermaid 图表与 LaTeX 行内、独立公式。渲染库和字体随程序本地打包；图表随主题切换，并可展开查看源码。
- **Markdown 排版**：支持嵌套列表、任务复选框、删除线、表格对齐、引用链接、网址和邮箱自动链接，以及完整标题层级。相对文件和图片链接按当前会话的工作目录解析。
...
- **结构化 Provider 会话**：原生支持 Codex/Claude 流式输出、审批、恢复、分叉、模型、推理强度、沙箱和上下文控制。Codex 选择会保存为 Glad 新会话默认值，并通过独立的显式操作写入 Codex 全局默认配置。
- **Codex 问题回复**：通过问题卡片回答待回复问题，手机端使用底部回答面板。异步回复可附带最多五张图片；本轮结束后仍可补答，直到新一轮请求开始。
- **Claude 结构化工作流**：可通过独立卡片回答 Claude 问题、跟踪 Todo/Task 进度及子代理，查看整合后的用量与上下文状态、压缩上下文，并从统一操作栏调用提示词、技能或命令。
- **Claude 历史浏览**：可排序、分页查看当前目录的 Claude 会话，预览最近消息后再明确恢复或分叉所选对话。
- **Codex 历史浏览**：可按创建或更新时间排序当前目录历史、继续加载旧会话，并在明确恢复或分叉前按需预览最近消息。
- **Codex 任务列表**：Glad 为 Codex 对话启用计划工具。Codex 创建计划后，可折叠悬浮卡片实时显示进度；本轮结束后，卡片放在用户命令与第一条回复之间。恢复或分叉对话时，会从本地 Codex 会话日志还原可解析的历史计划；没有记录或参数由动态代码生成、无法静态解析的计划会跳过。
- **Codex 自动标题**：通过现有 Codex Provider 的隔离临时请求生成短标题，同时保护手动名称。生成标题可能产生额外模型用量，失败不会阻塞正常对话。
...
- SkillHub API Token 默认使用 `~/.glad` 中自动生成的私有密钥加密，也可由部署环境指定外部密钥文件
