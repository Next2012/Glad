...
test('pages Codex history by raw JSON size without splitting complete turns', () => {
  const session = createSession();
  for (let turn = 1; turn <= 3; turn++) {
    session.append({ kind: 'turn-start', turnId: `turn-${turn}` });
    session.append({ kind: 'user', turnId: `turn-${turn}`, text: `question ${turn}` });
    session.append({ kind: 'assistant', turnId: `turn-${turn}`, text: String(turn).repeat(90 * 1024) });
    session.append({ kind: 'turn-end', turnId: `turn-${turn}`, status: 'completed' });
  }

  const latest = session.getMessagePage();
  assert.equal(latest.messages.filter(item => item.kind === 'turn-start').length, 2);
  assert.equal(latest.messages.find(item => item.kind === 'user').text, 'question 2');
  assert.equal(latest.hasMore, true);
  assert.ok(latest.bytes <= latest.maxBytes);

  const earlier = session.getMessagePage(latest.beforeId);
  assert.equal(earlier.messages.filter(item => item.kind === 'turn-start').length, 1);
  assert.equal(earlier.messages.find(item => item.kind === 'user').text, 'question 1');
  assert.equal(earlier.hasMore, false);

  const snapshot = session.snapshot();
  assert.deepEqual(snapshot.messages, latest.messages);
  assert.equal('messages' in snapshot.historyPage, false);
});

test('returns an oversized Codex turn as one complete history page', () => {
  const session = createSession();
  session.append({ kind: 'turn-start', turnId: 'large-turn' });
  session.append({ kind: 'user', turnId: 'large-turn', text: 'large request' });
  session.append({ kind: 'assistant', turnId: 'large-turn', text: 'x'.repeat(220 * 1024) });
  session.append({ kind: 'turn-end', turnId: 'large-turn', status: 'completed' });

  const page = session.getMessagePage();
