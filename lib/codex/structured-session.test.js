...
test('connects to the Codex app-server over a loopback WebSocket', async () => {
  const port = await CodexStructuredSession.reserveLoopbackPort();
  const server = new WebSocketServer({ host: '127.0.0.1', port });
  await new Promise(resolve => server.once('listening', resolve));

  const socket = await CodexStructuredSession.connectAppServerWebSocket(`ws://127.0.0.1:${port}`);
...
test('force kills the complete app-server process group on Unix', () => {
  let killed;
  const child = { pid: 123, kill() { assert.fail('must prefer the process group'); } };

  assert.equal(CodexStructuredSession.forceKillProcessTree(child, {
    platform: 'linux',
    killGroup(pid, signal) { killed = { pid, signal }; }
  }), true);

  assert.deepEqual(killed, { pid: -123, signal: 'SIGKILL' });
});

test('force kills the complete app-server process tree on Windows', () => {
  let invocation;
  const killer = new EventEmitter();

  assert.equal(CodexStructuredSession.forceKillProcessTree({ pid: 456 }, {
    platform: 'win32',
...
test('aborts and reports the current turn once when Codex reaches reconnect attempt 4/5', () => {
  const session = createSession();
  session.threadId = 'thread-reconnect';
  session.currentTurnId = 'turn-reconnect';
  session.status = 'running';
  session.threadTurns.set('thread-reconnect', {
  ...
  assert.equal(events.some(event => event.type === 'runtime-disconnected'), false);
  ...
  session.handleNotification('error', fourthAttempt);
  session.handleNotification('error', fourthAttempt);

  assert.deepEqual(requests, [{
    method: 'turn/interrupt',
    params: { threadId: 'thread-reconnect', turnId: 'turn-reconnect' }
  }]);
  assert.equal(events.filter(event => event.type === 'runtime-disconnected').length, 1);
  assert.deepEqual(events.find(event => event.type === 'runtime-disconnected'), {
    type: 'runtime-disconnected',
    activeTurn: true,
    turnId: 'turn-reconnect'
  });
  assert.equal(session.messages.some(item => item.text === 'Aborted after Codex reconnect attempt 4/5.'), true);
});

test('sends lightweight Codex message shells and loads tool details by id', () => {
  const session = createSession();
  session.threadId = 'root-thread';
  session.append({ kind: 'user', threadId: 'root-thread', turnId: 'turn-1', text: 'question' });
  const tool = session.append({ kind: 'tool', threadId: 'root-thread', turnId: 'turn-1',
    name: 'CodexBash', command: 'npm test', input: { command: 'npm test' },
    result: 'large output', toolStatus: 'completed' });
  const snapshot = session.snapshot();
  const shell = snapshot.messages.find(item => item.id === tool.id);
  assert.equal(shell.command, 'npm test');
  assert.equal(shell.hasDetail, true);
  assert.equal('result' in shell, false);
  assert.equal('input' in shell, false);
  assert.equal('historyPage' in snapshot, false);

  const detail = session.getMessageDetails({ ids: [tool.id] });
  assert.equal(detail.messages.length, 1);
  assert.equal(detail.messages[0].result, 'large output');
  assert.deepEqual(detail.messages[0].input, { command: 'npm test' });
  assert.equal(detail.messages[0].detailLoaded, true);
});

test('keeps subagent contents out of snapshots and loads them by thread id', () => {
  const session = createSession();
  session.threadId = 'root-thread';
  const subagent = session.append({ kind: 'assistant', threadId: 'subagent-1', turnId: 'sub-turn',
    text: 'private subagent detail' });
  const shell = session.snapshot().messages.find(item => item.id === subagent.id);
  assert.equal(shell.hasDetail, true);
  assert.equal('text' in shell, false);

  const detail = session.getMessageDetails({ threadId: 'subagent-1' });
  assert.equal(detail.messages.length, 1);
  assert.equal(detail.messages[0].text, 'private subagent detail');
});

test('replays resume and fork history as one lightweight reset event', () => {
  const session = createSession();
  session.threadId = 'root-thread';
  const events = [];
  session.on('event', event => events.push(event));

  session.restoreThreadHistory({ turns: [{
    id: 'turn-1',
    items: [
      { id: 'user-1', type: 'userMessage', content: [{ type: 'text', text: 'question' }] },
      { id: 'tool-1', type: 'commandExecution', command: 'npm test',
        aggregatedOutput: 'large restored output', status: 'completed' }
    ]
  }] }, 'Resumed thread');

  assert.equal(events.filter(event => event.type === 'message').length, 0);
  const reset = events.find(event => event.type === 'history-reset');
  assert.ok(reset);
  const tool = reset.messages.find(item => item.providerId === 'tool-1');
  assert.equal(tool.hasDetail, true);
  assert.equal('result' in tool, false);
});

test('keeps running tool updates lightweight while detail queries return the latest output', () => {
  const session = createSession();
  session.threadId = 'root-thread';
  const tool = session.append({ kind: 'tool', providerId: 'running-tool', threadId: 'root-thread',
    turnId: 'turn-1', name: 'CodexBash', result: 'first chunk', toolStatus: 'running' });
  const events = [];
  session.on('event', event => events.push(event));

  session.patch(tool.id, { result: 'first chunk\nsecond chunk' });

  const update = events.find(event => event.type === 'message-updated');
  assert.ok(update);
  assert.equal(update.message.hasDetail, true);
  assert.equal('result' in update.message, false);
  const detail = session.getMessageDetails({ ids: [tool.id] });
  assert.equal(detail.messages[0].result, 'first chunk\nsecond chunk');
});

test('merges provider user messages and maps Codex tools semantically', () => {
  const session = createSession();
  session.append({ kind: 'user', text: 'hello' });
  session.applyProviderItem({ id: 'user-1', type: 'userMessage', content: [
    { type: 'skill', name: 'repo-discovery', path: '/tmp/.agents/skills/repo-discovery/SKILL.md' },
  ...
  assert.deepEqual(session.messages.find(item => item.providerId === 'user-1').skills, [{
    name: 'repo-discovery', path: '/tmp/.agents/skills/repo-discovery/SKILL.md'
  }]);
...
test('lists enabled skills and sends only a selected skill returned by app-server', async () => {
  const requests = [];
  const session = createSession();
  const pdfSkill = {
    name: 'pdf',
    path: '/tmp/.agents/skills/pdf/SKILL.md',
  ...
  session.request = async (method, params) => {
    requests.push({ method, params });
    if (method === 'skills/list') return { data: [{ cwd: '/tmp', skills: [pdfSkill, {
      name: 'disabled', path: '/tmp/disabled/SKILL.md', description: '', enabled: false, scope: 'repo'
    }], errors: [] }] };
    if (method === 'thread/start') return { thread: { id: 'thread-skills' } };
    if (method === 'turn/start') return { turn: { id: 'turn-skills' } };
    return {};
  };

  const listed = await session.listSkills(true);
  assert.deepEqual(listed.skills, [pdfSkill]);
  await session.sendUserMessage('analyze this report', [], [
    { name: 'pdf', path: pdfSkill.path },
    { name: 'forged', path: '/etc/passwd' }
  ]);

  const listRequest = requests.find(item => item.method === 'skills/list');
  assert.deepEqual(listRequest.params, { cwds: ['/tmp'], forceReload: true });
  const turn = requests.find(item => item.method === 'turn/start');
  assert.deepEqual(turn.params.input, [
    { type: 'skill', name: 'pdf', path: pdfSkill.path },
    { type: 'text', text: 'analyze this report' }
  ]);
});

test('loads a SkillHub skill from the session-only root on every turn', async () => {
  const requests = [];
  const skill = {
    name: 'embedded-question-guide',
    path: '/run/glad-skill-sessions/session/skills/skill/SKILL.md'
  };
  const session = createSession({
    activeSkill: skill,
    extraSkillRoots: ['/run/glad-skill-sessions/session/skills']
  });
  session.ensureProcess = async () => {};
  session.request = async (method, params) => {
    requests.push({ method, params });
    if (method === 'skills/extraRoots/set') return {};
    if (method === 'skills/list') return { data: [{ cwd: '/tmp', skills: [{ ...skill, enabled: true }], errors: [] }] };
    if (method === 'thread/start') return { thread: { id: 'thread-skillhub' } };
    if (method === 'turn/start') return { turn: { id: 'turn-skillhub' } };
    return {};
  };

  await session.request('skills/extraRoots/set', { extraRoots: session.extraSkillRoots });
  await session.sendUserMessage('continue');
  const roots = requests.find(item => item.method === 'skills/extraRoots/set');
  assert.deepEqual(roots.params, { extraRoots: ['/run/glad-skill-sessions/session/skills'] });
  const listed = requests.find(item => item.method === 'skills/list');
  assert.deepEqual(listed.params, { cwds: ['/tmp'], forceReload: false });
  const turn = requests.find(item => item.method === 'turn/start');
  assert.deepEqual(turn.params.input, [
...
test('aborts every active Codex thread with its matching turn id', async () => {
  ...
  assert.equal(session.getControlState().canAbort, false);
  ...
  session.handleNotification('turn/completed', {
    threadId: 'root-thread', turn: { id: 'root-turn', status: 'interrupted' }
  });
  assert.equal(session.aborting, true);
  assert.equal(session.status, 'running');

  session.handleNotification('turn/completed', {
    threadId: 'child-thread', turn: { id: 'child-turn', status: 'interrupted' }
  });
  assert.equal(session.aborting, false);
  assert.equal(session.status, 'idle');
});

test('stops a stuck app-server and seals running tools after the abort grace period', () => {
  let killed = false;
  const session = createSession({
    abortGraceMs: 10,
    forceKillProcessTree(child) { child.kill(); }
  });
  session.process = { kill() { killed = true; } };
  session.processReady = Promise.resolve();
  session.threadId = 'root-thread';
  session.currentTurnId = 'root-turn';
  session.currentTurnStartedAt = Date.now() - 1000;
  session.status = 'running';
  session.threadTurns.set('root-thread', {
    turnId: 'root-turn', startedAt: session.currentTurnStartedAt, status: 'running'
  });
  session.append({ kind: 'tool', threadId: 'root-thread', turnId: 'root-turn',
    name: 'McpTool', toolStatus: 'running', startedAtMs: Date.now() - 500 });
  session.request = () => new Promise(() => {});

  assert.equal(session.abort(), true);
  assert.equal(session.forceAbortAfterTimeout(), true);

  assert.equal(killed, true);
  assert.equal(session.process, null);
  assert.equal(session.aborting, false);
  assert.equal(session.status, 'idle');
  assert.equal(session.needsThreadResume, true);
  assert.equal(session.messages.find(item => item.kind === 'tool').toolStatus, 'cancelled');
  assert.equal(session.messages.find(item => item.kind === 'turn-end').status, 'cancelled');
  assert.equal(session.messages.some(item => /did not stop within/.test(item.text)), true);
});

test('force kills and waits for a stuck app-server when the session is deleted during abort', async () => {
  ...
  session.currentTurnId = 'root-turn';
  session.status = 'running';
  session.threadTurns.set('root-thread', { turnId: 'root-turn', status: 'running' });
  session.request = () => new Promise(() => {});
  ...
  assert.equal(session.process, null);
  assert.equal(session.processShutdown, null);
  assert.equal(session.abortTimer, null);
  assert.equal(session.aborting, false);
});

test('resumes a retained thread before using a restarted app-server', async () => {
  const session = createSession();
  const requests = [];
  session.threadId = 'root-thread';
  session.needsThreadResume = true;
  session.model = 'gpt-test';
  session.hasModelOverride = true;
  session.request = async (method, params) => {
    requests.push({ method, params });
    return { thread: { id: 'root-thread', model: 'gpt-test' } };
  };

  assert.equal(await session.resumeThreadAfterProcessRestart(), true);

  assert.equal(session.needsThreadResume, false);
  assert.deepEqual(requests, [{
    method: 'thread/resume',
    params: { threadId: 'root-thread', cwd: '/tmp', model: 'gpt-test' }
  }]);
});

test('coalesces repeated resume requests and exposes a cancellable resuming state', async () => {
  const session = createSession();
  let finishResume;
  ...
  const first = session.resume('stored-thread');
  const second = session.resume('stored-thread');

  assert.equal(first, second);
  assert.equal(session.getControlState().resuming, true);
  assert.equal(session.getControlState().canAbort, true);
  await Promise.resolve();
  assert.equal(resumeRequests, 1);

  finishResume({ thread: { id: 'stored-thread' } });
  assert.equal(await first, true);
  assert.equal(session.getControlState().resuming, false);
  assert.equal(session.getControlState().canAbort, false);
});

test('disconnects a failed resume and allows a clean retry', async () => {
  const session = createSession();
  const disconnects = [];
  session.ensureProcess = async () => {};
  session.request = async method => {
    if (method === 'thread/resume') throw new Error('resume transport failed');
    return {};
  };
  session.disconnectProcess = async (force, reason) => { disconnects.push({ force, reason }); };

  await assert.rejects(session.resume('stored-thread'), /resume transport failed/);

  assert.equal(disconnects.length, 1);
  assert.equal(disconnects[0].force, true);
  assert.match(disconnects[0].reason.message, /resume transport failed/);
  assert.equal(session.resuming, false);
  assert.equal(session.needsThreadResume, true);
});

test('aborts an in-progress resume by stopping its app-server', async () => {
  const session = createSession();
  let rejectResume;
  session.ensureProcess = async () => {};
  session.request = method => method === 'thread/resume'
    ? new Promise((_resolve, reject) => { rejectResume = reject; })
    : Promise.resolve({});
  session.disconnectProcess = async (_force, reason) => { rejectResume(reason); };

  const resume = session.resume('stored-thread');
  await Promise.resolve();
  assert.equal(session.abort(), true);
  await assert.rejects(resume, /resume aborted by user/);
...
test('force stops the app-server when a lifecycle request times out', async () => {
  let killed = false;
  const session = createSession({
    forceKillProcessTree(child) {
      killed = true;
  ...
  child.pid = 321;
  session.process = child;
  session.rpcSocket = { readyState: 1, send() {}, terminate() {} };
  session.processReady = Promise.resolve();

  await assert.rejects(session.request('thread/resume', {}, {
    timeoutMs: 5,
    fatalOnTimeout: true
...
test('lists the two most recent user questions for each resume thread', async () => {
  ...
  session.request = async (method, params) => {
    ...
    if (params.threadId === 'current-thread') return { data: [
...
test('forks a Codex thread and hydrates the new session from provider history', async () => {
        ...
        turns: [{
          ...
          items: [
            { id: 'user-1', type: 'userMessage', content: [
              { type: 'skill', name: 'spec-init', path: '/tmp/.agents/skills/spec-init/SKILL.md' },
              { type: 'text', text: 'hello' }
            ] },
  ...
  assert.deepEqual(session.messages.find(item => item.providerId === 'user-1').skills, [{
    name: 'spec-init', path: '/tmp/.agents/skills/spec-init/SKILL.md'
  }]);
...
test('lists prompt history in pages, newest first, with a hard 200 item cap', async () => {
  ...
  session.request = async (method, params) => {
    ...
    const start = params.cursor === 'page-2' ? 104 : 204;
    const end = params.cursor === 'page-2' ? 5 : 105;
    return {
      data: Array.from({ length: start - end + 1 }, (_, offset) => {
      ...
      nextCursor: params.cursor === 'page-2' ? 'page-3' : 'page-2'
    };
  };
