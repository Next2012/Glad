...
test('merges provider user messages and maps Codex tools semantically', () => {
  ...
  session.applyProviderItem({ id: 'user-1', type: 'userMessage', content: [
    { type: 'skill', name: 'kiro-discovery', path: '/tmp/.agents/skills/kiro-discovery/SKILL.md' },
  ...
  assert.deepEqual(session.messages.find(item => item.providerId === 'user-1').skills, [{
    name: 'kiro-discovery', path: '/tmp/.agents/skills/kiro-discovery/SKILL.md'
  }]);
...
test('forks a Codex thread and hydrates the new session from provider history', async () => {
        ...
        turns: [{
          ...
          items: [
            { id: 'user-1', type: 'userMessage', content: [
              { type: 'skill', name: 'kiro-spec-init', path: '/tmp/.agents/skills/kiro-spec-init/SKILL.md' },
              { type: 'text', text: 'hello' }
            ] },
  ...
  assert.deepEqual(session.messages.find(item => item.providerId === 'user-1').skills, [{
    name: 'kiro-spec-init', path: '/tmp/.agents/skills/kiro-spec-init/SKILL.md'
  }]);
