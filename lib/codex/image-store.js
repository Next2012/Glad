...
class CodexImageStore {
  ...
  assertSession(session) {
    ...
    if (session.kind !== 'codex-structured' || session.presentation !== 'structured') {
      throw inputError('Image attachments are available only in Codex chat mode');
    }
  }
  ...
  resolve(session, attachmentIds = []) {
    if (!session || session.kind !== 'codex-structured') throw inputError('Codex session not found', 404);
