...
test('deletes a structured Codex session through its own lifecycle', async () => {
  const manager = createManager();
  const session = addCodexSession(manager, 'codex-delete-test');
  ...
  const deleted = await manager.kill(session.id);
...
test('stops structured sessions without requiring a PTY manager', () => {
  const manager = createManager();
  const session = addCodexSession(manager, 'codex-stop-all-test');
  let killCount = 0;
  session.kill = () => { killCount += 1; };
