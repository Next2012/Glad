...
test('creates an independent Glad session for a selected Codex thread fork', async () => {
  ...
  const source = {
    ...
    append: item => sourceEvents.push(item)
  };
  const target = {
    id: 'target-session',
    name: 'Codex Work Fork',
    forkFrom: async threadId => {
      assert.equal(threadId, 'thread-selected');
      target.threadId = 'thread-forked';
      return { threadId: target.threadId };
    }
  };
  manager.sessions.set(source.id, source);
  manager.createCodexStructuredSession = options => {
    assert.equal(options.workingDirectory, '/tmp/project');
    assert.deepEqual(options.codexOptions, {
      model: 'gpt-test', effort: 'high', permissionMode: 'on-request', sandboxMode: 'workspace-write'
    });
    manager.sessions.set(target.id, target);
    return target;
  };
  ...
  assert.equal(target.parentSessionId, source.id);
  assert.equal(target.forkedFromThreadId, 'thread-selected');
  assert.match(sourceEvents[0].text, /Forked a new session/);
});
