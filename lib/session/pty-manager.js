...
class PTYManager {
  ...
  handlePTYOutput(data) {
    ...
    if (filtered.length > 0) {
      ...
      // FIX: If we're in a custom size (from mobile), local terminal might look messy.
      // We still write to stdout, but we ensure basic sanitization.
      if (!this.outputPaused) {
        // Only write to stdout if the size is somewhat reasonable for local display
        // to prevent extreme ANSI artifacts when mobile resizes PTY to 40 cols
