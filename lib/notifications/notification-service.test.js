...
test('sends one connection interruption for a reconnect-triggered runtime disconnect', async () => {
  const { sessionManager, sent, service } = createHarness();
  const session = createCodexSession();
  sessionManager.sessions.set(session.id, session);
  const disconnected = {
    sessionId: session.id,
    session,
    event: { type: 'runtime-disconnected', activeTurn: true, turnId: 'turn-reconnect' }
  };

  sessionManager.emit('codex-event', disconnected);
  sessionManager.emit('codex-event', disconnected);
  await flushNotifications();

  assert.equal(sent.length, 1);
  assert.equal(sent[0].title, '连接中断｜Codex工作');
  service.stop();
});
