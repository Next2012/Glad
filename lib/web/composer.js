        ...
        let composerPendingSend = null;

        function composerSocketConnected() {
            return Boolean(currentSocket && currentSocket.readyState === WebSocket.OPEN);
        }

        function composerSendInFlight() {
            return Boolean(composerPendingSend);
        }

        function composerExecutionInProgress() {
            if (composerPendingSend) return true;
        ...
        function syncComposerSendState() {
            ...
            const disconnected = (isClaudeSession() || isCodexSession()) && !composerSocketConnected();
            const executing = composerExecutionInProgress();
            sendButton.disabled = disconnected || executing;
            sendButton.title = disconnected ? 'Session connection unavailable'
                : executing ? 'Wait for the current run to finish' : 'Send';
            sendButton.setAttribute('aria-label', disconnected ? 'Send disabled while disconnected'
                : executing ? 'Send disabled while running' : 'Send');
        }

        function beginComposerSend(payload, draft) {
            if (!composerSocketConnected()) {
                syncComposerSendState();
                alert('The session is not connected. Your message has been kept.');
                return false;
            }
            const clientMessageId = crypto.randomUUID?.() || `${Date.now()}-${Math.random().toString(16).slice(2)}`;
            composerPendingSend = { ...draft, clientMessageId };
            renderComposerAttachments();
            try {
                currentSocket.send(JSON.stringify({ ...payload, clientMessageId }));
            } catch (error) {
                composerPendingSend = null;
                renderComposerAttachments();
                syncComposerSendState();
                alert(`Unable to send: ${error.message}`);
                return false;
            }
            syncComposerSendState();
            return true;
        ...
        function handleComposerSendResult(message) {
            if (!composerPendingSend || message.clientMessageId !== composerPendingSend.clientMessageId) return;
            const pending = composerPendingSend;
            composerPendingSend = null;
            if (message.accepted) {
                if (inputEl.value === pending.text) {
                    inputEl.value = '';
                    inputEl.style.height = '38px';
                }
                const imageIds = new Set(pending.imageIds || []);
                const fileIds = new Set(pending.fileIds || []);
                selectedImageAttachments = selectedImageAttachments.filter(item => !imageIds.has(item.id));
                selectedFileAttachments = selectedFileAttachments.filter(item => !fileIds.has(item.id));
                if (pending.skillPath && selectedCodexSkill?.path === pending.skillPath) selectedCodexSkill = null;
                if (pending.claudeSkill && selectedClaudeSkill?.name === pending.claudeSkill) selectedClaudeSkill = null;
                renderComposerAttachments();
                renderComposerSkillPrefix();
                if (isCodexSession()) renderCodexChat();
            } else {
                if (!inputEl.value && pending.text) {
                ...
                renderComposerAttachments();
                alert(`Unable to send: ${message.error || 'Unknown provider error'}`);
            }
            syncComposerSendState();
        }

        function handleComposerSocketOpen() {
            syncComposerSendState();
        }

        function handleComposerSocketClose() {
            if (composerPendingSend) {
                composerPendingSend = null;
                renderComposerAttachments();
                alert('The session connection closed before the message was accepted. Your draft has been kept.');
            }
            syncComposerSendState();
        }
        ...
        async function uploadImageFiles(files) {
            ...
            for (const file of batch) {
                ...
                renderComposerAttachments();
                try {
                    const upload = uploadImageInChunks(sessionId, file, progress => {
                    ...
                    if (index < 0) {
                        await fetchWithTimeout(`/api/sessions/${sessionId}/attachments/images/${encodeURIComponent(attachment.id)}`, { method: 'DELETE' });
                        continue;
                    }
                    selectedImageAttachments[index] = { ...attachment, name: file.name || attachment.name, sessionId };
                    renderComposerAttachments();
                } catch (e) {
        ...
        async function uploadAttachmentFiles(files) {
            ...
            for (const file of batch) {
                ...
                renderComposerAttachments();
                try {
                    const upload = uploadFileInChunks(sessionId, file, progress => {
                        pending.progress = progress;
                        pending.status = `${progress}%`;
                        renderComposerAttachments();
                    });
                    ...
                    if (index < 0) {
                        await fetchWithTimeout(`/api/sessions/${sessionId}/attachments/files/${encodeURIComponent(attachment.id)}`, { method: 'DELETE' });
                        continue;
                    }
                    selectedFileAttachments[index] = { ...attachment, name: file.name || attachment.name, sessionId };
                    renderComposerAttachments();
                } catch (e) {
        ...
        function performSend() {
            ...
            const hasStructuredContent = Boolean(val.trim() || readyImageAttachments.length || readyFileAttachments.length);
            if (hasStructuredContent && isClaudeSession()) {
                beginComposerSend({
                    type: 'claude-input',
                    text: val,
                    ...
                    skills: selectedClaudeSkill ? [{ name: selectedClaudeSkill.name, command: selectedClaudeSkill.command }] : []
                }, {
                    text: val,
                    imageIds: readyImageAttachments.map(item => item.id),
                    fileIds: readyFileAttachments.map(item => item.id),
                    claudeSkill: selectedClaudeSkill?.name || ''
                });
                return;
            }
            if (hasStructuredContent && isCodexSession()) {
                ...
                beginComposerSend({
                    ...
                    imageIds: readyImageAttachments.map(item => item.id),
                    fileIds: readyFileAttachments.map(item => item.id),
                    skillPath: selectedCodexSkill?.path || ''
                });
                return;
            }
            if (!hasStructuredContent && (isClaudeSession() || isCodexSession())) return;
