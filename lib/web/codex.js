        ...
        function codexReadyForInput() {
            return codexState.status === 'idle'
                ...
                && !codexResumeInFlight && !codexForkInFlight;
        }
        ...
        function renderCodexPermission(request, inline = false) {
            ...
            const result = labels[request.decision] || request.status;
            const content = `${detail ? `<div>${codexText(detail)}</div>` : ''}${pending ? `<div class="claude-permission-actions"><button class="small-btn primary" onclick="respondCodexPermission('${escapeHtml(request.id)}', 'approved')">Yes</button><button class="small-btn" onclick="respondCodexPermission('${escapeHtml(request.id)}', 'approved_for_session')">Yes, for session</button><button class="small-btn danger" onclick="respondCodexPermission('${escapeHtml(request.id)}', 'abort')">Stop and explain</button></div>` : `<div class="codex-permission-result">${escapeHtml(result)}</div>`}`;
            const summary = `<summary class="${inline ? 'codex-inline-permission-summary' : 'claude-tool-header'}"><strong>${escapeHtml(request.title || 'Permission required')}</strong><span class="codex-permission-summary-state">${escapeHtml(pending ? 'Approval required' : result)}</span></summary>`;
            if (inline) return `<details class="codex-inline-permission" data-codex-key="permission-${permissionId}" data-codex-permission-id="${permissionId}">${summary}<div class="codex-inline-permission-body">${content}</div></details>`;
            return `<details class="claude-tool claude-tool-card claude-permission codex-permission-card" data-codex-key="permission-${permissionId}" data-codex-permission-id="${permissionId}">${summary}<div class="claude-tool-body">${content}</div></details>`;
        }

        function codexPermissionLayout(requests, items, rootThreadId) {
            const before = new Map();
            const after = new Map();
            const unplaced = [];
            const add = (map, item, request) => {
                const key = String(item.id || item.providerId || '');
                if (!key) { unplaced.push(request); return; }
                if (!map.has(key)) map.set(key, []);
                map.get(key).push(request);
            };
            for (const request of requests) {
                const requestThreadId = String(request.threadId || rootThreadId || '');
                const requestTurnId = String(request.turnId || '');
                let candidates = items.filter(item => {
                    const itemThreadId = String(item.threadId || rootThreadId || '');
                    return itemThreadId === requestThreadId && (!requestTurnId || String(item.turnId || '') === requestTurnId);
                });
                if (!candidates.length) {
                    candidates = items.filter(item => String(item.threadId || rootThreadId || '') === requestThreadId);
                }
                if (!candidates.length) { unplaced.push(request); continue; }
                const createdAt = Number(request.createdAt || 0);
                if (createdAt > 0) {
                    const preceding = candidates.filter(item => {
                        const timestamp = Number(item.createdAt || item.startedAtMs || 0);
                        return timestamp > 0 && timestamp <= createdAt;
                    });
                    if (preceding.length) { add(after, preceding[preceding.length - 1], request); continue; }
                    const following = candidates.find(item => Number(item.createdAt || item.startedAtMs || 0) > createdAt);
                    if (following) { add(before, following, request); continue; }
                }
                add(after, candidates[candidates.length - 1], request);
            }
            const byTime = (left, right) => Number(left.createdAt || 0) - Number(right.createdAt || 0);
            for (const values of [...before.values(), ...after.values()]) values.sort(byTime);
            unplaced.sort(byTime);
            return { before, after, unplaced };
        }

        function renderCodexPermissionList(requests, usedPermissions) {
            return (requests || []).map(request => {
                usedPermissions.add(request.id);
                return renderCodexPermission(request);
            }).join('');
        }
        ...
        function formatCodexTokens(value) {
            return formatAgentTokens(value);
        }
        ...
        function codexRateLimitLabel(window, fallback) {
            const minutes = Number(window?.windowDurationMins || 0);
            if (minutes === 300) return '5h limit';
            if (minutes === 10080) return 'Weekly limit';
            if (minutes > 0 && minutes % 10080 === 0) return `${minutes / 10080}w limit`;
            if (minutes > 0 && minutes % 1440 === 0) return `${minutes / 1440}d limit`;
            if (minutes > 0 && minutes % 60 === 0) return `${minutes / 60}h limit`;
            if (minutes > 0) return `${minutes}m limit`;
            return `${fallback} limit`;
        }
        function codexRateLimitValue(window) {
            ...
            return `${remaining}% left${window.resetsAt ? ` · resets ${formatCodexReset(window.resetsAt)}` : ''}`;
        }
        function renderCodexRateLimits(item) {
            const legacy = item.rateLimits && typeof item.rateLimits === 'object' ? item.rateLimits : null;
            const byId = item.rateLimitsByLimitId && typeof item.rateLimitsByLimitId === 'object'
                ? item.rateLimitsByLimitId : null;
            const buckets = byId ? Object.entries(byId).map(([limitId, value]) => ({
                ...(value || {}), limitId: value?.limitId || limitId
            })) : [];
            if (!buckets.length && legacy) buckets.push(legacy);
            return buckets.flatMap(bucket => {
                const bucketName = bucket.limitName || (bucket.limitId && bucket.limitId !== 'codex' ? bucket.limitId : '');
                return [['primary', 'Primary'], ['secondary', 'Secondary']].map(([key, fallback]) => {
                    const window = bucket[key];
                    if (!window) return '';
                    const windowLabel = codexRateLimitLabel(window, fallback);
                    const label = bucketName ? `${bucketName} · ${windowLabel}` : windowLabel;
                    return codexStatusItem(label, codexRateLimitValue(window));
                }).filter(Boolean);
            }).join('');
        }
        ...
        function isCodexSubagentItem(item, rootThreadId = codexState.threadId) {
            return Boolean(item?.threadId && rootThreadId && item.threadId !== rootThreadId);
        }
        function renderCodexSubagentGroup(threadId, items, permissionById, usedPermissions, standalonePermissions = [], interactive = true, markdownOptions = {}) {
            ...
            const deferContent = needsDetail && !items.some(item => item.detailLoaded || item.kind === 'question');
            const body = [];
            const permissionLayout = codexPermissionLayout(standalonePermissions, content, threadId);
            if (needsDetail) {
            ...
            for (let i = 0; !deferContent && i < content.length;) {
                ...
                body.push(renderCodexPermissionList(permissionLayout.before.get(String(item.id || item.providerId || '')), usedPermissions));
                if (item.kind === 'tool') {
                    const group = [];
                    ...
                    for (const groupedItem of group.slice(1)) {
                        body.push(renderCodexPermissionList(permissionLayout.before.get(String(groupedItem.id || groupedItem.providerId || '')), usedPermissions));
                    }
                    body.push(renderCodexToolGroup(group, permissionById, usedPermissions, turnEnd));
                    for (const groupedItem of group) {
                        body.push(renderCodexPermissionList(permissionLayout.after.get(String(groupedItem.id || groupedItem.providerId || '')), usedPermissions));
                    }
                    continue;
                }
                if (item.kind === 'question') {
                    body.push(renderCodexQuestion(item, interactive));
                } else if (item.kind === 'task-plan') {
                    body.push(renderCodexPlan(item));
                } else if (item.kind === 'assistant' || item.kind === 'user') {
                    if (item.text) body.push(`<div class="codex-subagent-message${item.kind === 'user' ? ' task' : ''}">${renderMarkdown(item.text, markdownOptions)}</div>`);
                } else if (item.text) {
                    body.push(`<div class="codex-subagent-message">${codexText(item.text)}</div>`);
                }
                body.push(renderCodexPermissionList(permissionLayout.after.get(String(item.id || item.providerId || '')), usedPermissions));
                i += 1;
            }
            body.push(renderCodexPermissionList(deferContent ? standalonePermissions : permissionLayout.unplaced, usedPermissions));
        ...
        function renderCodexContextMeter(context) {
            return renderAgentContextMeter(context);
        }
        ...
        function renderComposerSkillPrefix() {
            ...
            if (!container) return;
            const claudeSkill = isClaudeSession() ? selectedClaudeSkill : null;
            const codexSkill = isCodexSession() ? selectedCodexSkill : null;
            const selected = claudeSkill || codexSkill;
            const visible = selected && (isClaudeSession() ? claudeStatus === 'idle' : codexReadyForInput());
            const wasVisible = container.classList.contains('active');
            container.classList.toggle('active', Boolean(visible));
            container.innerHTML = visible
                ? `<div class="codex-skill-bubble" role="status" aria-label="Selected skill: ${escapeHtml(selected.name)}"><span class="codex-skill-bubble-name">Skill · ${escapeHtml(selected.name)}</span><button type="button" class="codex-skill-bubble-close" onclick="${claudeSkill ? 'clearClaudeSkillSelection' : 'clearCodexSkillSelection'}()" title="Remove selected skill" aria-label="Remove selected skill">×</button></div>`
                : '';
        ...
        function syncCodexDom(current, next) {
            ...
            if (gladMarkdownRich.preserveDiagram(current, next)) return;
            ...
            const planFinished = current.classList?.contains('codex-plan-card')
                && current.dataset.planPhase !== next.dataset.planPhase && next.dataset.planPhase !== 'running';
            const preserveOpen = current.tagName === 'DETAILS' && currentKey === nextKey && !planFinished;
        ...
        function buildCodexConversationHtml(messages = codexMessages, permissions = codexPendingPermissions, state = codexState, markdownOptions = {}) {
            const parts = [];
            const { floatingPlan, plansAfterUser, plansBeforeReply, placedPlans } = codexPlanLayout(messages, state.threadId);
            const permissionById = new Map(permissions.map(item => [String(item.id), item]));
            const usedPermissions = new Set();
            const matchedPermissionIds = new Set(messages.filter(item => item.kind === 'tool')
                .map(item => String(item.providerId || '')).filter(id => permissionById.has(id)));
            const standalonePermissions = permissions.filter(item => !matchedPermissionIds.has(String(item.id)));
            const turnEndById = new Map(messages.filter(item => item.kind === 'turn-end' && item.turnId)
                .map(item => [String(item.turnId), item]));
            const lastAssistantByTurn = new Map();
            for (const item of messages) {
                if (item.kind === 'assistant' && item.turnId && !isCodexSubagentItem(item, state.threadId)) {
                    lastAssistantByTurn.set(String(item.turnId), item.id);
                }
            }
            const subagentItems = new Map();
            for (const item of messages) {
                if (!isCodexSubagentItem(item, state.threadId)) continue;
            ...
            const visible = messages.filter(item => !['reasoning', 'turn-start', 'turn-end'].includes(item.kind)
                && !placedPlans.has(item));
            const rootVisible = visible.filter(item => item !== floatingPlan && !isCodexSubagentItem(item, state.threadId));
            const rootStandalonePermissions = standalonePermissions.filter(item => !item.threadId || String(item.threadId) === String(state.threadId || ''));
            const rootPermissionLayout = codexPermissionLayout(rootStandalonePermissions, rootVisible, state.threadId);
            for (let i = 0; i < visible.length;) {
                const item = visible[i];
                if (item === floatingPlan) { i += 1; continue; }
                if (isCodexSubagentItem(item, state.threadId)) {
                    const threadId = String(item.threadId);
                    if (!renderedSubagents.has(threadId)) {
                        renderedSubagents.add(threadId);
                        const ownedPermissions = standalonePermissions.filter(request => String(request.threadId || '') === threadId);
                        parts.push(renderCodexSubagentGroup(threadId, subagentItems.get(threadId) || [], permissionById, usedPermissions, ownedPermissions, messages === codexMessages, markdownOptions));
                    }
                    i += 1;
                    continue;
                }
                parts.push(renderCodexPermissionList(rootPermissionLayout.before.get(String(item.id || item.providerId || '')), usedPermissions));
                if (item.kind === 'tool') {
                    const tools = [];
                    const turnId = item.turnId;
                    while (i < visible.length && visible[i].kind === 'tool' && visible[i].turnId === turnId) tools.push(visible[i++]);
                    for (const groupedItem of tools.slice(1)) {
                        parts.push(renderCodexPermissionList(rootPermissionLayout.before.get(String(groupedItem.id || groupedItem.providerId || '')), usedPermissions));
                    }
                    parts.push(renderCodexToolGroup(tools, permissionById, usedPermissions,
                        turnEndById.get(String(turnId || ''))));
                    for (const groupedItem of tools) {
                        parts.push(renderCodexPermissionList(rootPermissionLayout.after.get(String(groupedItem.id || groupedItem.providerId || '')), usedPermissions));
                    }
                    continue;
                }
                if (item.kind === 'assistant') {
                    if (plansBeforeReply.has(item)) parts.push(renderCodexPlan(plansBeforeReply.get(item)));
                    ...
                    parts.push(`<div class="codex-message-block assistant" data-codex-key="message-${escapeHtml(item.id || '')}"><div class="codex-message assistant claude-md">${renderMarkdown(item.text || '', markdownOptions)}</div>${renderCodexMessageMeta(item, true, context)}</div>`);
                }
                else if (item.kind === 'user') parts.push(`<div class="codex-message-block user" data-codex-key="message-${escapeHtml(item.id || '')}">${renderCodexMessageSkills(item)}<div class="codex-message user claude-md">${renderMarkdown(item.text || '', markdownOptions)}${renderCodexMessageAttachments(item)}</div>${renderCodexMessageMeta(item)}</div>`);
                else if (item.kind === 'question') parts.push(renderCodexQuestion(item, messages === codexMessages));
                else if (item.kind === 'task-plan') parts.push(renderCodexPlan(item));
                ...
                if (plansAfterUser.has(item)) parts.push(renderCodexPlan(plansAfterUser.get(item)));
                parts.push(renderCodexPermissionList(rootPermissionLayout.after.get(String(item.id || item.providerId || '')), usedPermissions));
                i += 1;
            }
            parts.push(renderCodexPermissionList(rootPermissionLayout.unplaced, usedPermissions));
            for (const request of permissions) if (!usedPermissions.has(request.id)) parts.push(renderCodexPermission(request));
            const working = `<div class="codex-working-indicator" role="status" aria-label="Codex is working" title="Codex is working"${state.status === 'running' ? '' : ' style="display:none"'}></div>`;
            const expandPlan = messages === codexMessages && document.getElementById('codex-chat-container')?.clientWidth >= 640;
            const dock = `<div class="codex-plan-dock${floatingPlan ? ' has-plan' : ''}">${floatingPlan ? renderCodexPlan(floatingPlan, true, expandPlan) : working}</div>`;
            const emptyState = state.status === 'running'
                ? 'Codex 正在生成回复…' : 'Send a message to start Codex.';
            return `<div class="codex-conversation">${dock}${parts.join('') || (floatingPlan ? '' : `<div class="codex-message event">${emptyState}</div>`)}</div>`;
        }
        ...
        function commitCodexChatRender() {
            ...
            template.innerHTML = buildCodexConversationHtml();
        ...
        function installCodexLazyDetailHandler() {
            ...
            installCodexQuestionHandlers(container);
        ...
        function codexDetailIsOpen(message) {
            ...
            if (isCodexSubagentItem(message)) {
                return Array.from(container.querySelectorAll('.codex-subagent-group[open]'))
                    .some(group => group.dataset.codexKey === `subagent-${message.threadId}`);
            }
            return Array.from(container.querySelectorAll('.codex-work-group[open][data-codex-detail-ids]'))
        ...
        function renderCodexStateBar() {
            ...
            const parts = [];
            const attention = [];
            const questions = codexMessages.filter(item => item.kind === 'question' && item.questionStatus === 'pending').length;
            if (questions) attention.push(`<button type="button" class="session-attention-pill approval" onclick="jumpToCodexQuestion()" aria-label="Jump to pending question">? ${questions} awaiting answer ↓</button>`);
            if (codexState.aborting) parts.push('<span class="claude-state-pill warn">Stopping Codex…</span>');
            else if (codexState.forking || codexForkInFlight) parts.push('<span class="claude-state-pill">Forking conversation…</span>');
            else if (codexState.resuming || codexResumeInFlight) parts.push('<span class="claude-state-pill">Resuming conversation…</span>');
            if (pending) attention.push(`<button type="button" class="session-attention-pill approval codex-approval-jump" onclick="jumpToCodexApproval()" title="Jump to pending approval" aria-label="Jump to pending approval"><span aria-hidden="true">!</span>${pending} approval${pending === 1 ? '' : 's'}<span aria-hidden="true">↓</span></button>`);
        ...
        function jumpToActiveCodexSubagent(retry = true) {
            const groups = Array.from(document.getElementById('codex-chat-container')?.querySelectorAll('.codex-subagent-group') || []);
            const activeThreadIds = Array.isArray(codexState.activeSubagentThreadIds)
                ? codexState.activeSubagentThreadIds.map(value => String(value || '')).filter(Boolean)
                : [];
            const latestActiveThreadId = activeThreadIds[activeThreadIds.length - 1] || '';
            let target = latestActiveThreadId
                ? groups.find(group => group.dataset.codexThreadId === latestActiveThreadId) || null
                : null;
            // Keep compatibility with older Glad servers that only report a count.
            if (!activeThreadIds.length) target = groups[groups.length - 1] || null;
            if (!target) {
        ...
        async function loadAndFocusCodexApproval(request) {
            ...
            requestAnimationFrame(() => {
                if (activeSessionId === sessionId && codexPendingPermissions.some(item => String(item.id) === permissionId && item.status === 'pending')) {
                    focusCodexApproval(permissionId);
                }
            });
        }

        function focusCodexApproval(permissionId, retry = true) {
            const target = Array.from(document.getElementById('codex-chat-container')?.querySelectorAll('[data-codex-permission-id]') || [])
        ...
        function renderCodexModelPanel() {
            ...
            const efforts = (candidate.efforts?.length ? candidate.efforts : [candidate.defaultEffort || 'medium'])
                .map(codexEffortValue).filter(Boolean);
        ...
        function codexEffortValue(value) {
            if (value && typeof value === 'object') {
                return String(value.reasoningEffort || value.reasoning_effort || value.value || value.id || '');
            }
            return String(value || '');
        }
        ...
        function chooseCodexModelEffort(model, effort) {
            ...
            void sendCodexSettings({ model, effort });
            updateTerminalControlsHeight();
        }

        function applyCodexEvent(event) {
            if (!event) return;
            if (event.type === 'session-renamed') { applySessionName(activeSessionId, event.name); return; }
            let statePatch = event.state || null;
            if (event.type === 'message' && event.message) {
                ...
                codexPendingPermissions = [...codexPendingPermissions.filter(item => item.id !== event.request.id), event.request];
            }
            else if (event.type === 'permission-updated' && event.request) {
                codexPendingPermissions = codexPendingPermissions.map(item => item.id === event.request.id ? event.request : item);
            }
            if (event.type === 'permission-request' || event.type === 'permission-updated') {
                const pendingPermissionCount = codexPendingPermissions.filter(item => item.status === 'pending').length;
                statePatch = { ...(statePatch || {}), pendingPermissionCount };
                if (event.type === 'permission-request' && pendingPermissionCount > 0) statePatch.status = 'waiting_approval';
                if (event.type === 'permission-updated' && pendingPermissionCount === 0
                    && (statePatch.status || codexState.status) === 'waiting_approval') statePatch.status = 'running';
            }
            if (statePatch) applyCodexState(statePatch, { providerStateReceived: Boolean(event.state) });
            else renderCodexChat();
        }

        function sendCodexSettings(settings) {
            const sessionId = activeSessionId;
            if (!sessionId) return Promise.resolve(false);
            const sequence = ++codexSettingsRequestSequence;
            const save = async () => {
                try {
                    const response = await fetchWithTimeout(`/api/sessions/${sessionId}/codex-settings`, {
                        method: 'PATCH',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify(settings)
                    }, 15000);
                    const data = await response.json();
                    if (!response.ok || !data.success) throw new Error(data.error || 'Could not save Codex settings');
                    if (activeSessionId === sessionId && sequence === codexSettingsRequestSequence && data.state) {
                        applyCodexState(data.state, { providerStateReceived: true });
                    }
                    return true;
                } catch (error) {
                    showAppToast(error.message || 'Could not save Codex settings');
                    return false;
                }
            };
            codexSettingsSaveQueue = codexSettingsSaveQueue.then(save, save);
            return codexSettingsSaveQueue;
        }
        function updateCodexSettingsFromControls() {
            ...
            void sendCodexSettings({ permissionMode, sandboxMode });
        }
        async function saveCodexGlobalDefaults() {
            const sessionId = activeSessionId;
            if (!sessionId) return false;
            const permission = codexState.permissionMode && codexState.permissionMode !== 'default'
                ? codexState.permissionMode : codexState.effectivePermissionMode;
            const sandbox = codexState.sandboxMode && codexState.sandboxMode !== 'default'
                ? codexState.sandboxMode : codexState.effectiveSandboxMode;
            const summary = `${codexState.model || 'current model'} / ${codexState.effort || 'current effort'}, ${sandbox || 'current sandbox'}, ${permission || 'current approval policy'}`;
            if (!confirm(`Set these as Codex global defaults for all new Codex sessions?\n\n${summary}`)) return false;
            const button = document.getElementById('codex-global-defaults-btn');
            if (button) button.disabled = true;
            try {
                const response = await fetchWithTimeout(`/api/sessions/${sessionId}/codex-global-defaults`, {
                    method: 'POST'
                }, 15000);
                const data = await response.json();
                if (!response.ok || !data.success) throw new Error(data.error || 'Could not update Codex global defaults');
                showAppToast('Codex global defaults updated');
                return true;
            } catch (error) {
                showAppToast(error.message || 'Could not update Codex global defaults');
                return false;
            } finally {
                if (button && activeSessionId === sessionId) button.disabled = false;
            }
        }
        ...
        function flushPendingCodexStatus(socket = currentSocket) {
            if (!codexStatusPending || socket !== currentSocket || socket?.readyState !== 1) return false;
            codexStatusPending = false;
            socket.send(JSON.stringify({ type: 'codex-status' }));
            return true;
        }
        function requestCodexStatus() {
            codexStatusPending = true;
            return flushPendingCodexStatus();
        }
        ...
        async function toggleCodexResumePanel() {
            ...
            updateTerminalControlsHeight();
            if (!codexResumePanelOpen) { cancelCodexThreadPicker(panel); return; }
        ...
        async function toggleCodexForkPanel() {
            ...
            updateTerminalControlsHeight();
            if (!codexForkPanelOpen) { cancelCodexThreadPicker(panel); return; }
        ...
        async function selectCodexResumeThread(threadId) {
            if (codexResumeInFlight || codexForkInFlight || codexState.resuming) return;
            ...
            let failure = '';
            try {
                const res = await fetchWithTimeout(`/api/sessions/${sessionId}/codex-resume`, {
                ...
                if (activeSessionId !== sessionId) { applyCodexState({}); return; }
        ...
        async function selectCodexForkThread(threadId) {
            ...
            updateTerminalControlsHeight();
            try {
                const res = await fetchWithTimeout(`/api/sessions/${sessionId}/codex-fork`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ threadId }) }, 60000);
                const data = await res.json();
                if (!res.ok || !data.success) throw new Error(data.error || 'Unable to fork Codex thread');
                if (activeSessionId !== sessionId) return;
                codexForkPanelOpen = false;
                panel.classList.remove('active');
                panel.innerHTML = '';
                ...
                if (activeSessionId !== sessionId) return;
                panel.innerHTML = `<div class="codex-history-notice" role="alert">${escapeHtml(error.message)} <button type="button" class="small-btn" onclick="loadCodexThreadPanel(document.getElementById('codex-fork-panel'), 'fork')">Back to sessions</button></div>`;
            } finally {
                codexForkInFlight = false;
                if (activeSessionId !== sessionId) { applyCodexState({}); return; }
                applyCodexState({});
                updateTerminalControlsHeight();
            }
