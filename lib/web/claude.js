        ...
        function setClaudeModeEnabled(enabled) {
            ...
            // Anchor Send to the active toolbar, not the whole scrollable composer.
            const sendButton = document.getElementById('send-btn');
            const actionContainerId = enabled ? 'claude-control-panel'
                : codexChat ? 'codex-control-panel' : 'terminal-controls';
            const actionContainer = document.getElementById(actionContainerId);
            if (sendButton.parentElement !== actionContainer) actionContainer.appendChild(sendButton);
        ...
        function shortModelLabel(value, resolved) {
            ...
            const compact = target
                ...
                .replace(/-20\d{6,}.*$/, '')
                .slice(0, 9);
            if (source === 'default') return `M:${compact || 'Def'}`;
        ...
        function readableClaudeModelName(value) {
            const normalized = String(value || '').replace(/^claude-/, '').replace(/-20\d{6,}.*$/, '');
        ...
        function fullModelLabel(value, resolved, label) {
            const source = String(value || 'default');
            if (source === 'default') {
                const target = readableClaudeModelName(resolved);
                return target ? `Default (${target})` : 'Default';
            }
        ...
        function syncClaudePickerButtons() {
            const mappings = [
                ['permission', 'claude-permission-select', 'claude-permission-picker-btn', 'Mode'],
        ...
        async function toggleClaudePicker(type) {
            ...
            if (type === 'model' && !(Array.isArray(claudeState.models) && claudeState.models.length)) {
                await refreshClaudeRuntimeConfig();
            }
            closeClaudeActionPanels();
        ...
        async function chooseClaudePickerOption(type, value) {
            const select = document.getElementById(selectIdForPicker(type));
            if (!select) return;
            if (type === 'permission' && value === 'bypassPermissions'
                && !confirm('Bypass lets Claude run tools without approval. Use it only in a trusted, isolated workspace. Continue?')) return;
            select.value = value;
        ...
        function applyClaudeRuntimeConfig(config, options = {}) {
            ...
            if (modelEl) {
                const current = options.model || modelEl.value || claudeState.model || config.defaultModel || 'default';
                modelEl.innerHTML = '';
                const models = config.models.map(item => typeof item === 'string'
                    ? { value: item, label: fullModelLabel(item), resolved: '' }
                    : {
                        ...item,
                        value: item?.value || item?.id || item?.modelId,
                        label: item?.label || item?.displayName || item?.value || item?.id || item?.modelId,
                        resolved: item?.resolved || item?.resolvedModel || ''
                    }).filter(item => item && item.value);
                if (config.defaultModel && !models.some(item => item.value === config.defaultModel)) {
                    models.push({ value: config.defaultModel, label: config.defaultModel, resolved: config.defaultModel });
                }
                models.forEach(item => {
        ...
        function applyClaudeState(state = {}, options = {}) {
            ...
            if (Array.isArray(state.models) && state.models.length) {
                applyClaudeRuntimeConfig({
                    ...(claudeRuntimeConfig || {}),
                    models: state.models
                }, { model: claudeState.model || 'default' });
            }
            ...
            const statusBtn = document.getElementById('claude-status-btn');
            if (statusBtn) {
                statusBtn.disabled = claudeStatusPending || claudeStatus === 'thinking';
                setActionButtonLabel(statusBtn, claudeStatusPending ? 'Loading' : 'Status');
            }
            const compactBtn = document.getElementById('claude-compact-btn');
            if (compactBtn) {
                compactBtn.disabled = !claudeState.canCompact || claudeState.compacting || claudeStatus === 'thinking';
                setActionButtonLabel(compactBtn, claudeState.compacting ? 'Compacting' : 'Compact');
            }
            const skillsBtn = document.getElementById('claude-skills-btn');
            if (skillsBtn) skillsBtn.classList.toggle('primary', Boolean(selectedClaudeSkill));
        ...
        function renderClaudeMessageMeta(message, context = null, finalOnly = false) {
            const time = renderClaudeMessageTime(message, finalOnly);
            const meter = renderAgentContextMeter(context);
        ...
        function renderClaudeStateBar() {
            ...
            const parts = [];
            const attention = [];
            const questions = claudeMessages.filter(item => item.kind === 'question' && item.questionStatus === 'pending').length;
            const completedTools = new Set(claudeMessages.filter(item => item.kind === 'tool-result').map(item => String(item.toolUseId || '')));
            const agents = claudeMessages.filter(item => item.kind === 'tool' && ['Agent', 'Task'].includes(item.name)
                && !completedTools.has(String(item.toolUseId || ''))).length;
            const taskPlan = claudeMessages.findLast(item => item.kind === 'task-plan' && item.planTurnStatus === 'running');
            if (questions) attention.push(`<button type="button" class="session-attention-pill approval" onclick="jumpToClaudeQuestion()" aria-label="Jump to pending Claude question">? ${questions} awaiting answer ↓</button>`);
            if (pending) {
            ...
            if (mode !== 'default') {
                attention.unshift(`<button type="button" class="session-attention-pill mode ${escapeHtml(mode)}" onclick="toggleClaudePicker('permission')" title="Change Claude permission mode" aria-label="Change Claude permission mode">${escapeHtml(permissionModeLabel(mode))}</button>`);
            }
            if (agents) attention.push(`<button type="button" class="session-attention-pill subagent" onclick="jumpToClaudeSubagent()" aria-label="Jump to active Claude subagent">↳ ${agents} agent${agents === 1 ? '' : 's'} running</button>`);
            if (taskPlan?.plan?.length) {
                const completed = taskPlan.plan.filter(step => step.status === 'completed').length;
                attention.push(`<button type="button" class="session-attention-pill" onclick="jumpToClaudeTasks()" aria-label="Jump to Claude tasks">${completed}/${taskPlan.plan.length} tasks</button>`);
            }
        ...
        function jumpToClaudeSubagent() {
            commitClaudeChatRender();
            const groups = document.querySelectorAll('#claude-chat-container .codex-subagent-group');
        ...
        function jumpToClaudeTasks() {
            commitClaudeChatRender();
            const target = document.querySelector('#claude-chat-container .agent-task-card');
            if (!target) return false;
            target.open = true;
            target.scrollIntoView({ behavior: 'smooth', block: 'center' });
            return true;
        ...
        function focusClaudeApproval(permissionId, retry = true) {
            const target = Array.from(document.getElementById('claude-chat-container')?.querySelectorAll('[data-claude-permission-id]') || [])
        ...
        function requestClaudeStatus() {
            if (!currentSocket || currentSocket.readyState !== 1 || claudeStatusPending) return;
            prepareClaudeInfoRequest();
            closeClaudeActionPanels();
            claudeStatusPending = true;
            applyClaudeState({});
            currentSocket.send(JSON.stringify({ type: 'claude-status' }));
        }

        function compactClaudeContext() {
            if (!currentSocket || currentSocket.readyState !== 1 || !claudeState.canCompact || claudeState.compacting) return;
            closeClaudePicker();
            closeClaudeActionPanels();
            applyClaudeState({ compacting: true, canCompact: false });
            currentSocket.send(JSON.stringify({ type: 'claude-compact' }));
        }

        async function updateClaudeSettingsFromControls() {
            if (!(Array.isArray(claudeState.models) && claudeState.models.length)) {
                await refreshClaudeRuntimeConfig();
            }
            const settings = {
        ...
        async function toggleClaudeResumePanel() {
            ...
            claudeForkPanelOpen = false;
            const forkPanel = document.getElementById('claude-fork-panel');
            forkPanel.classList.remove('active');
            cancelClaudeHistoryPicker(forkPanel);
            const panel = document.getElementById('claude-resume-panel');
            panel.classList.toggle('active', claudeResumePanelOpen);
            updateTerminalControlsHeight();
            if (claudeResumePanelOpen) await loadClaudeResumeSessions(panel, 'resume');
            else cancelClaudeHistoryPicker(panel);
        }
        ...
        async function toggleClaudeForkPanel() {
            ...
            cancelClaudeHistoryPicker(resumePanel);
        ...
        async function loadClaudeResumeSessions(panel = document.getElementById('claude-resume-panel'), action = 'resume') {
            ...
            await renderClaudeHistoryPicker(panel, action);
        }

        async function selectClaudeResumeSession(id) {
			...
			updateTerminalControlsHeight();
			try {
				const response = await fetchWithTimeout(`/api/sessions/${encodeURIComponent(sessionId)}/claude-resume`, {
					method: 'POST',
					headers: { 'Content-Type': 'application/json' },
					body: JSON.stringify({ resumeSessionId: id })
				}, 60000);
				const data = await response.json();
				if (!response.ok || !data.success) throw new Error(data.error || 'Unable to resume Claude conversation');
				if (activeSessionId !== sessionId) return;
				applyClaudeState({ resumeSessionId: data.claudeSessionId || id, claudeSessionId: data.claudeSessionId || id });
				claudeResumePanelOpen = false;
        ...
        function renderWorkGroup(items, context) {
            ...
            return `<details class="codex-work-group claude-work-group-compact" data-claude-key="group-${escapeHtml(key)}"${running ? ' open' : ''}>
                <summary>${escapeHtml(label)} · ${items.length} ${items.length > 1 ? 'tools' : 'tool'}${running ? '<span class="claude-tool-status running">running</span>' : ''}</summary>
                <div class="codex-work-group-body">${items.map(item => renderDisplayItem(item, context)).join('')}</div>
            </details>`;
        }
        ...
        function renderUserMessage(message, markdownOptions = {}) {
            ...
            const skills = Array.isArray(message.skills) && message.skills.length
                ? `<div class="codex-message-skills">${message.skills.map(skill => `<span class="codex-message-skill">Skill · ${escapeHtml(skill.name || 'unknown')}</span>`).join('')}</div>` : '';
            if (parsed.kind === 'command') {
                const args = parsed.args ? `<div class="claude-message user">${renderMarkdown(parsed.args, markdownOptions)}</div>` : '';
                return `${args}<div class="claude-message user"><span class="claude-command-chip">/${escapeHtml(parsed.commandName)}</span>${skills}${attachments}</div>`;
            }
            const content = parsed.text ? renderMarkdown(parsed.text, markdownOptions) : '';
            return `<div class="claude-message user">${content}${skills}${attachments}</div>`;
        }
        ...
        function renderClaudeStatusCard(message) {
            if (message.error) {
                return `<div class="codex-status-card claude-status-card error" data-claude-key="message-${escapeHtml(message.id || '')}"><div class="codex-status-title">Claude status</div><div class="codex-status-value">${escapeHtml(message.error)}</div></div>`;
            }
            const usage = message.usage || {};
            const context = message.context || {};
            const contextLabel = context.maxTokens
                ? `${formatTokenCount(context.usedTokens)} / ${formatTokenCount(context.maxTokens)} · ${Number(context.usedPercent || 0)}% used` : '';
            const rateLimits = (usage.rateLimits || []).map(limit => {
                const labels = { session: 'Current session', weekly_all: 'Current week', weekly: 'Current week' };
                const used = Math.max(0, Math.min(100, Number(limit.usedPercent || 0)));
                const resetDate = limit.resetsAt ? new Date(limit.resetsAt) : null;
                const reset = limit.resetsLabel || (resetDate && !Number.isNaN(resetDate.getTime()) ? resetDate.toLocaleString() : '');
                return claudeUsageCardItem(labels[limit.kind] || limit.group || 'Usage limit', `${100 - used}% left${reset ? ` · resets ${reset}` : ''}`);
            }).join('');
            return `<div class="codex-status-card claude-status-card" data-claude-key="message-${escapeHtml(message.id || '')}">
                <div class="codex-status-title">${escapeHtml(message.title || 'Claude status')}</div><div class="codex-status-grid">
                    ${claudeUsageCardItem('Model', context.model || claudeState.model)}
                    ${claudeUsageCardItem('Session cost', usage.totalCostUsd == null ? '' : formatClaudeMoney(usage.totalCostUsd))}
                    ${claudeUsageCardItem('Tokens', usage.inputTokens == null && usage.outputTokens == null ? '' : `${formatTokenCount(usage.inputTokens)} in · ${formatTokenCount(usage.outputTokens)} out`)}
                    ${claudeUsageCardItem('Cache', usage.cacheReadTokens == null && usage.cacheWriteTokens == null ? '' : `${formatTokenCount(usage.cacheReadTokens)} read · ${formatTokenCount(usage.cacheWriteTokens)} write`)}
                    ${claudeUsageCardItem('Context', contextLabel)}
                    ${claudeUsageCardItem('Available', context.remainingTokens == null ? '' : `${formatTokenCount(context.remainingTokens)} tokens`)}
                    ${rateLimits}
                </div></div>`;
        }

        function renderClaudeCompaction(message) {
            const running = message.compactionStatus === 'running';
            const failed = message.compactionStatus === 'failed';
            return `<div class="codex-compaction-card${running ? ' running' : ''}${failed ? ' error' : ''}" data-claude-key="message-${escapeHtml(message.id || '')}"><span class="codex-compaction-icon" aria-hidden="true">⇣</span><div><div class="codex-compaction-title">${running ? 'Compacting context' : failed ? 'Context compaction failed' : 'Context compacted'}</div>${message.text ? `<div class="codex-compaction-meta">${escapeHtml(message.text)}</div>` : ''}</div></div>`;
        }

        function renderClaudeSubagentGroup(tool, items, context) {
            if (!items?.length) return '';
            const finished = context?.completedToolUseIds?.has(String(tool.toolUseId || ''));
            const bodyContext = { ...context, subagentItems: new Map() };
            const body = buildClaudeDisplayItems(items).map(item => renderDisplayItem(item, bodyContext)).filter(Boolean).join('');
            return `<details class="claude-work-group codex-subagent-group" data-claude-key="subagent-${escapeHtml(tool.toolUseId || tool.id || '')}"><summary><span class="claude-work-group-title">${finished ? 'Subagent finished' : 'Subagent working'} · ${escapeHtml(tool.input?.subagent_type || tool.input?.description || tool.name || 'Agent')}</span></summary><div class="claude-work-group-body">${body}</div></details>`;
        }

        function renderDisplayItem(message, context = null) {
            if (!message) return '';
            if (message.kind === 'user') return `<div class="claude-message-block user" data-claude-key="message-${escapeHtml(message.id || '')}">${renderUserMessage(message, context?.markdownOptions)}${renderClaudeMessageTime(message)}</div>`;
            if (message.kind === 'assistant') {
                const isFinal = context?.lastAssistantByTurn?.get(String(message.turnId || '')) === message.id;
                const turnContext = isFinal ? context?.turnEndById?.get(String(message.turnId || ''))?.context : null;
                return `<div class="claude-message-block assistant" data-claude-key="message-${escapeHtml(message.id || '')}"><div class="claude-message assistant">${renderMarkdown(textFromClaudeMessage(message), context?.markdownOptions)}</div>${renderClaudeMessageMeta(message, turnContext, true)}</div>`;
            }
            if (message.kind === 'tool') {
                ...
                return renderClaudeTool(message, permission)
                    + renderClaudeSubagentGroup(message, context?.subagentItems?.get(String(message.toolUseId || '')), context);
            }
            ...
            if (message.kind === 'status') return renderClaudeStatusCard(message);
            if (message.kind === 'compaction') return renderClaudeCompaction(message);
            if (message.kind === 'question') return renderClaudeQuestion(message, context?.interactive !== false);
            if (message.kind === 'task-plan') return renderAgentPlan(message, { initiallyOpen: message.planTurnStatus === 'running', label: 'Tasks', keyPrefix: 'claude-plan' });
            if (message.kind === 'reasoning') return `<details class="claude-work-group" data-claude-key="message-${escapeHtml(message.id || '')}"><summary><span class="claude-work-group-title">Thinking</span></summary><div class="claude-work-group-body">${renderMarkdown(message.text || '', context?.markdownOptions)}</div></details>`;
        ...
        function renderClaudePermissionActions(req) {
            ...
            if (Array.isArray(req.suggestions) && req.suggestions.length) {
                parts.push(`<button class="small-btn primary" onclick="respondClaudePermission('${id}', 'allow-remember')">Allow &amp; remember</button>`);
            }
            if (isClaudeEditTool(toolName) || isClaudeExitPlanTool(toolName) || req.canAllowEdits) {
                parts.push(`<button class="small-btn primary" onclick="respondClaudePermission('${id}', 'allow-edits')">${isClaudeExitPlanTool(toolName) ? 'Approve plan & implement' : 'Allow edits'}</button>`);
            }
            ...
            parts.push(`<button class="small-btn danger" onclick="respondClaudePermission('${id}', 'deny')">${isClaudeExitPlanTool(toolName) ? 'Continue planning' : 'Deny'}</button>`);
            return parts.join('');
        }

        function renderClaudePermission(req, inline = false) {
            const id = escapeHtml(String(req.id || ''));
            const plan = isClaudeExitPlanTool(req.toolName) && req.input?.plan
                ? `<div class="claude-plan-review">${renderMarkdown(req.input.plan)}</div>` : '';
            const body = `${req.reason ? `<div>${escapeHtml(req.reason)}</div>` : ''}
                ...
                ${plan || renderToolSection('Input', req.input || {})}
                <textarea class="claude-permission-feedback" rows="2" placeholder="Optional feedback if denied"></textarea>
            ...
            if (inline) {
                return `<div class="claude-inline-permission" data-claude-permission-id="${id}">
                    <div class="claude-inline-permission-title">${escapeHtml(isClaudeExitPlanTool(req.toolName) ? 'Plan ready for review' : req.title || req.toolName || 'Permission required')}</div>${body}
                </div>`;
            }
            return `<div class="claude-tool claude-permission" data-claude-key="permission-${id}" data-claude-permission-id="${id}">
                <div class="claude-tool-header"><span class="claude-tool-icon" data-icon="!"></span><strong>${escapeHtml(isClaudeExitPlanTool(req.toolName) ? 'Plan ready for review' : req.title || req.toolName || 'Permission required')}</strong></div>
        ...
        function buildClaudeConversationHtml(messages = claudeMessages, permissions = claudePendingPermissions, status = claudeStatus, markdownOptions = {}) {
            const permissionByToolUseId = new Map(permissions
                .filter(item => item.status === 'pending' && item.toolUseId)
                .map(item => [String(item.toolUseId), item]));
            const subagentItems = new Map();
            for (const item of messages) {
                const parent = String(item.parentToolUseId || '');
                if (!parent) continue;
                if (!subagentItems.has(parent)) subagentItems.set(parent, []);
                subagentItems.get(parent).push(item);
            }
            const completedToolUseIds = new Set(messages.filter(item => item.kind === 'tool-result')
            ...
            for (const item of messages) {
                if (item.kind === 'assistant' && item.turnId && !item.parentToolUseId) lastAssistantByTurn.set(String(item.turnId), item.id);
            }
            const floatingTask = messages.findLast(item => item.kind === 'task-plan' && item.planTurnStatus === 'running');
            const rootMessages = messages.filter(item => !item.parentToolUseId && item !== floatingTask);
            const context = { permissionByToolUseId, usedPermissions: new Set(), markdownOptions, subagentItems, completedToolUseIds, turnEndById, lastAssistantByTurn, interactive: true };
            const parts = buildClaudeDisplayItems(rootMessages).map(item => renderDisplayItem(item, context)).filter(Boolean);
            for (const req of permissions.filter(item => item.status === 'pending')) {
                if (!context.usedPermissions.has(req.id)) parts.push(renderClaudePermission(req));
            }
            const working = `<div class="claude-working-indicator" role="status" aria-label="Claude is working" title="Claude is working"${status === 'thinking' ? '' : ' style="display:none"'}></div>`;
            const taskDock = `<div class="codex-plan-dock${floatingTask ? ' has-plan' : ''}">${floatingTask ? renderAgentPlan(floatingTask, { floating: true, initiallyOpen: true, label: 'Tasks', keyPrefix: 'claude-plan' }) : ''}</div>`;
            return `<div class="claude-conversation">${working}${taskDock}${parts.join('') || '<div class="claude-message event">Send a message to start Claude.</div>'}</div>`;
        }
        ...
        function commitClaudeChatRender() {
            ...
            template.innerHTML = buildClaudeConversationHtml();
            const next = template.content.firstElementChild;
            const current = container.firstElementChild;
            if (!current) container.appendChild(next);
            else syncClaudeDom(current, next);
            installClaudeQuestionHandlers(container);
            renderClaudeStateBar();
        ...
        function applyClaudeEvent(event) {
            if (!event) return;
			let statePatch = event.state ? { ...event.state } : null;
            if (event.type === 'message' && event.message) {
                claudeMessages.push(event.message);
                if (event.message.kind === 'status') claudeStatusPending = false;
            } else if (event.type === 'message-updated' && event.message) {
                const index = claudeMessages.findIndex(item => item.id === event.message.id);
                if (index >= 0) claudeMessages[index] = event.message;
                else claudeMessages.push(event.message);
            } else if (event.type === 'status') {
                claudeStatus = event.status || 'idle';
				statePatch = { ...(statePatch || {}), status: claudeStatus };
				...
				claudePendingPermissions = event.request.status === 'pending'
					? claudePendingPermissions.map(item => item.id === event.request.id ? event.request : item)
					: claudePendingPermissions.filter(item => item.id !== event.request.id);
            } else if (event.type === 'history-reset' && Array.isArray(event.messages)) {
                claudeMessages = event.messages;
				document.getElementById('claude-chat-container')?.replaceChildren();
            } else if (event.type === 'state' && event.state) {
				statePatch = { ...event.state };
            }
			if (event.type === 'permission-request' || event.type === 'permission-updated') {
				const pendingPermissionCount = claudePendingPermissions.filter(item => item.status === 'pending').length;
				...
				if (event.type === 'permission-updated' && pendingPermissionCount === 0
					&& (statePatch.status || claudeState.status) === 'waiting_approval') statePatch.status = 'thinking';
			}
			if (statePatch) applyClaudeState(statePatch, { providerStateReceived: Boolean(event.state) || event.type === 'status' });
			else applyClaudeState({
				status: claudeStatus,
				pendingPermissionCount: claudePendingPermissions.filter(item => item.status === 'pending').length,
				canAbort: ['thinking', 'waiting_input', 'waiting_approval'].includes(claudeStatus)
			});
        }
        ...
        function respondClaudePermission(id, actionOrApproved) {
            ...
            const card = Array.from(document.querySelectorAll('[data-claude-permission-id]'))
                .find(element => element.dataset.claudePermissionId === String(id));
            const message = card?.querySelector('.claude-permission-feedback')?.value?.trim() || '';
