        ...
        function renderDisplayItem(message, context = null) {
            ...
            if (message.kind === 'assistant') return `<div class="claude-message-block assistant" data-claude-key="message-${escapeHtml(message.id || '')}"><div class="claude-message assistant">${renderMarkdown(textFromClaudeMessage(message), context?.markdownOptions)}</div>${renderClaudeMessageTime(message, true)}</div>`;
            if (message.kind === 'tool') {
        ...
        function buildClaudeConversationHtml(messages = claudeMessages, permissions = claudePendingPermissions, status = claudeStatus, markdownOptions = {}) {
            ...
            const context = { permissionByToolUseId, usedPermissions: new Set(), markdownOptions };
            const parts = buildClaudeDisplayItems(messages).map(item => renderDisplayItem(item, context)).filter(Boolean);
            ...
            return `<div class="claude-conversation">${working}${parts.join('') || '<div class="claude-message event">Send a message to start Claude.</div>'}</div>`;
        }
