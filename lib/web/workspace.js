        ...
        function exitTiledMode() {
            if (!isTiledMode()) return;
            const restoreId = tiledWorkspace.focusedSessionId || tiledWorkspace.previousSessionId;
            const restore = tiledWorkspace.sessions.find(session => session.id === restoreId);
            returnToTiles();
