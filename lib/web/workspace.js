        ...
        function tileStatusLabel(status) {
            const labels = {
                ...
                waiting_approval: 'Waiting', waiting_input: 'Waiting for answer', error: 'Error'
            };
        ...
        function tileConversationHtml(session, preview) {
            ...
            if (session.toolKey === 'claude-code') {
                return buildClaudeConversationHtml(preview.messages, preview.permissions, preview.status, { sessionId: session.id });
            }
            if (session.toolKey === 'codex') {
                return buildCodexConversationHtml(preview.messages, preview.permissions, preview.state, { sessionId: session.id });
            }
        ...
        function makeTileConversationReadOnly(container) {
            ...
            for (const element of container.querySelectorAll('button, a, input, select, textarea, summary, [contenteditable], [tabindex]')) {
                // Expanding a progress card is a local viewing action.
                if (element.matches('.codex-plan-summary, .markdown-diagram-source > summary')) continue;
        ...
        function captureTileSurfaceState(container, preview) {
            ...
            preview.view = {
                ...
                planPhases: Object.fromEntries(Array.from(container.querySelectorAll('.codex-plan-card'))
                    .map(card => [tileDetailKey(card), card.dataset.planPhase]))
            };
        }
        ...
        function restoreTileSurfaceState(container, preview) {
            ...
            for (const details of container.querySelectorAll('details')) {
                ...
                const planFinished = details.matches('.codex-plan-card')
                    && view.planPhases?.[key] !== details.dataset.planPhase && details.dataset.planPhase !== 'running';
                if (key && openKeys.has(key) && !planFinished) details.open = true;
            }
        ...
        function tileTimedTagsHtml(preview) {
            ...
            return (preview?.timedInputs || [])
                ...
                .map(item => {
                    ...
                    return `<span class="timed-tag${failed ? ' failed' : ''}" title="${escapeHtml(title || '')}">${timedTagContent(item)}</span>`;
                }).join('');
        }
        ...
        function applyTileEvent(sessionId, event) {
            ...
            if (event.type === 'message' && event.message) preview.messages.push(event.message);
				...
				preview.permissions = event.request.status === 'pending'
					? preview.permissions.map(item => item.id === event.request.id ? event.request : item)
					: preview.permissions.filter(item => item.id !== event.request.id);
            }
        ...
        function connectTilePreview(session) {
            ...
            socket.onmessage = message => {
                ...
                if ((payload.type === 'claude-snapshot' || payload.type === 'codex-snapshot') && payload.snapshot) {
                    ...
                    if (payload.snapshot.hasUnreadCompletion) {
                        noteSessionCompletion(session.id, payload.snapshot.completionRevision);
                    }
                }
                if ((payload.type === 'claude-event' || payload.type === 'codex-event') && payload.event) {
                    ...
                    if (payload.event.type === 'completion') {
                        noteSessionCompletion(session.id, payload.event.revision);
                    }
                }
            };
            socket.onclose = () => {
        ...
        function renderTileGrid() {
            ...
            grid.innerHTML = sessions.map(session => {
                ...
                const completionRevision = session.hasUnreadCompletion
                    ? (Number(session.completionRevision) || 0)
                    : null;
                const preview = tiledWorkspace.previewStates.get(session.id) || createTilePreviewState(session);
                tiledWorkspace.previewStates.set(session.id, preview);
                ...
                return `<article class="tile-session-window" data-session-id="${escapeHtml(session.id)}">
                    <header class="tile-session-header" title="Drag to reorder sessions on this page">
                        ...
                        <div class="tile-session-actions">
                            ...
                            <button class="btn-join tile-connect-button" type="button" onclick="openTileDialog(decodePathValue('${encodedId}'), decodePathValue('${encodedName}'), decodePathValue('${encodedTool}'), ${completionRevision === null ? 'null' : completionRevision})">Connect</button>
        ...
        function refreshTiledSessionsFromList(sessions) {
            ...
            if (window.gladSessionOrder.deferRefresh(sessions)) return;
            sessions = window.gladSessionOrder.sort(sessions);
        ...
        function exitTiledMode() {
            ...
            if (restore) joinSession(
                restore.id,
                restore.name,
                restore.toolKey,
                restore.hasUnreadCompletion ? (Number(restore.completionRevision) || 0) : null
            );
            else showLobby();
        }

        function openTileDialog(id, name, toolKey, completionRevision = null) {
            if (!isTiledMode()) return;
            tiledWorkspace.focusedSessionId = id;
            tiledWorkspace.mode = 'focused';
            document.body.classList.add('tile-focus-open');
            setTileLayoutMenuOpen(false);
            joinSession(id, name, toolKey, completionRevision);
