        ...
        const pendingCompletionRevisions = new Map();
        const seenCompletionRevisions = new Map();
        ...
        let claudeStatusPending = false;
        let claudePromptPanelOpen = false;
        let claudeSkillPanelOpen = false;
        let claudeCommandPanelOpen = false;
        let selectedClaudeSkill = null;
        let claudeState = {
        ...
        function createDefaultCodexState() {
            return {
                ...
                activeSubagentCount: 0, activeSubagentThreadIds: []
            };
        }
        let codexMessages = [];
        ...
        let codexSettingsSaveQueue = Promise.resolve(true);
        let codexSettingsRequestSequence = 0;
        ...
        function renderSessionCard(session) {
            ...
            const completionRevision = Number(session.completionRevision) || 0;
            const completionAckRevision = session.hasUnreadCompletion ? completionRevision : null;
            const completionSeenHere = completionRevision > 0
                && (seenCompletionRevisions.get(session.id) || 0) >= completionRevision;
            const hasUnreadCompletion = Boolean(session.hasUnreadCompletion)
                && !completionSeenHere
                && !isSessionCompletionVisible(session.id);
        ...
        function isSessionCompletionVisible(id) {
            if (document.hidden) return false;
            if (id === activeSessionId && document.getElementById('terminal-view')?.classList.contains('active')) {
                return true;
            }
            if (!window.gladWorkspace?.isTiledMode() || document.body.classList.contains('tile-focus-open')) {
                return false;
            }
            return Boolean(document.querySelector(
                `.tile-session-window[data-session-id="${CSS.escape(String(id))}"]`
            ));
        }

        function noteSessionCompletion(id, revision) {
            const normalized = Number(revision) || 0;
            if (normalized > 0) {
                const previous = pendingCompletionRevisions.get(id) || 0;
                if (normalized < previous) return;
                pendingCompletionRevisions.set(id, normalized);
            }
            if (isSessionCompletionVisible(id)) void markCompletionRead(id, normalized);
        }

        async function markCompletionRead(id, revision = 0) {
            if (revision > 0) {
                seenCompletionRevisions.set(id, Math.max(
                    seenCompletionRevisions.get(id) || 0,
                    revision
                ));
            }
            try {
                const response = await fetchWithTimeout('/api/sessions/' + id + '/completion/read', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(revision > 0 ? { revision } : {})
                });
                const result = await response.json();
                if (!response.ok || !result.success) throw new Error(result.error || `HTTP ${response.status}`);
                if (result.cleared === false) return false;
                if (revision === 0 || pendingCompletionRevisions.get(id) === revision) {
                    pendingCompletionRevisions.delete(id);
                }
                document.querySelector(
                    `.session-card[data-session-id="${CSS.escape(String(id))}"] .completion-dot`
                )?.remove();
                return true;
            } catch (e) {
        ...
        document.addEventListener('visibilitychange', () => {
            ...
            for (const [id, revision] of pendingCompletionRevisions) {
                if (isSessionCompletionVisible(id)) void markCompletionRead(id, revision);
            }
        });
