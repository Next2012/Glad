        ...
        function renderTimedTags() {
            ...
            rail.innerHTML = activeItems.map(item => {
                ...
                return `<button class="timed-tag${failed ? ' failed' : ''}${item.id === editingTimedInputId ? ' active' : ''}" title="${escapeHtml(title)}" onclick="editTimedInput(decodePathValue('${encodedId}'))">${failed ? 'Failed' : escapeHtml(formatCountdown(item.sendAt))}</button>`;
            }).join('');
        }
