        ...
        function timedTagContent(item) {
            const label = item.status === 'failed' ? 'Failed' : formatCountdown(item.sendAt);
            return `<svg class="action-icon" aria-hidden="true"><use href="#icon-schedule"></use></svg><span>${escapeHtml(label)}</span>`;
        }

        function renderTimedTags() {
            const rail = document.getElementById('timed-tag-rail');
            const now = Date.now();
            const activeItems = timedInputs.filter(item => item.sendAt > now || item.status === 'failed');
            rail.innerHTML = activeItems.map(item => {
                ...
                return `<button type="button" class="timed-tag${failed ? ' failed' : ''}${item.id === editingTimedInputId ? ' active' : ''}" aria-label="Edit scheduled message: ${escapeHtml(item.text || '')}" aria-pressed="${item.id === editingTimedInputId}" title="${escapeHtml(title)}" onclick="editTimedInput(decodePathValue('${encodedId}'))">${timedTagContent(item)}</button>`;
            }).join('');
        }
        ...
        function resetTimedEditor(options = {}) {
            ...
            document.getElementById('timed-editor-actions').hidden = true;
        ...
        function editTimedInput(id) {
            ...
            document.getElementById('timed-editor-actions').hidden = false;
