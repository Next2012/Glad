        function applySessionName(id, name) {
            if (!name) return;
            if (id === activeSessionId) document.getElementById('session-title').textContent = name;
            const card = document.querySelector(`.session-card[data-session-id="${CSS.escape(id)}"] .session-name`);
            if (card) { card.textContent = name; card.title = name; }
            if (typeof tiledWorkspace !== 'undefined') {
                const session = tiledWorkspace.sessions.find(item => item.id === id);
                if (session) session.name = name;
            }
            const tile = document.querySelector(`.tile-session-window[data-session-id="${CSS.escape(id)}"] .tile-session-title-row strong`);
            if (tile) { tile.textContent = name; tile.title = name; }
        }

        function joinSession(id, sessionName, toolKey = null, completionRevision = null) {
            ...
            if (completionRevision !== null) markCompletionRead(id, completionRevision);
            loadTimedInputs();
        ...
        let structuredReconnectDelay = 1000;

        function reconnectStructuredSession(sessionId, socket, connect) {
            if (currentSocket !== socket || activeSessionId !== sessionId) return;
            const delay = structuredReconnectDelay;
            structuredReconnectDelay = Math.min(delay * 2, 10000);
            // 网关可能关闭空闲 WebSocket；继续当前会话时重新取得完整快照。
            setTimeout(() => {
                if (currentSocket === socket && activeSessionId === sessionId) connect(sessionId);
            }, delay);
        }
        ...
        async function initClaudeSession(sessionId) {
            ...
            claudeStatusPending = false;
            claudePromptPanelOpen = false;
            claudeSkillPanelOpen = false;
            claudeCommandPanelOpen = false;
            ...
            for (const id of ['claude-prompt-panel', 'claude-skill-panel', 'claude-command-panel']) {
            ...
            renderClaudeChat();
            connectClaudeSocket(sessionId);
        }

        function connectClaudeSocket(sessionId) {
            currentSocket = new WebSocket(sessionWebSocketUrl(sessionId));
            ...
            socket.onclose = () => {
                if (currentSocket === socket) {
                    handleComposerSocketClose();
                    reconnectStructuredSession(sessionId, socket, connectClaudeSocket);
                }
            };
            socket.onerror = () => {
            ...
            socket.onmessage = (e) => {
                ...
                if (msg.type === 'claude-snapshot' && msg.snapshot) {
                    ...
                    renderClaudeChat();
                    if (msg.snapshot.hasUnreadCompletion) {
                        noteSessionCompletion(sessionId, msg.snapshot.completionRevision);
                    }
                }
                if (msg.type === 'claude-event') {
                    applyClaudeEvent(msg.event);
                    if (msg.event?.type === 'completion') noteSessionCompletion(sessionId, msg.event.revision);
                }
                if (msg.type === 'exit') showLobby();
            };
        }
        ...
        function initCodexSession(sessionId) {
            ...
            codexSettingsSaveQueue = Promise.resolve(true);
            codexSettingsRequestSequence += 1;
            ...
            connectCodexSocket(sessionId);
        }

        function connectCodexSocket(sessionId) {
            currentSocket = new WebSocket(sessionWebSocketUrl(sessionId));
            const socket = currentSocket;
            socket.onopen = () => {
                if (currentSocket !== socket) return;
            ...
            socket.onclose = () => {
                if (currentSocket === socket) {
                    handleComposerSocketClose();
                    renderCodexChat();
                    reconnectStructuredSession(sessionId, socket, connectCodexSocket);
                }
            };
            socket.onerror = () => {
                if (currentSocket === socket) syncComposerSendState();
            ...
            socket.onmessage = (e) => {
                ...
                if (msg.type === 'codex-snapshot' && msg.snapshot) {
                    applySessionName(sessionId, msg.snapshot.name);
