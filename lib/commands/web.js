...
async function webCommand(options) {
  ...
  function markSessionInput(session, data) {
    ...
    logSessionDiagnostics('session-input', session, {
      inputSeq: session.inputSeq,
      inputPreview: previewText(data)
    }, { compact: true });
  }

  function createManagedSession({ toolKey, workingDirectory, name }) {
    logger.info(`Creating session: toolKey=${toolKey || ''}, workingDirectory=${workingDirectory || '(default)'}`);
    const tool = getToolByKey(toolKey);
    if (!tool) {
      const err = new Error('Invalid tool');
      err.statusCode = 400;
      logger.error(`Create session failed: invalid toolKey=${toolKey || ''}`);
      throw err;
    }
    ...
    const sessionDir = workingDirectory && String(workingDirectory).trim()
      ? path.resolve(baseDir, String(workingDirectory).trim())
      : baseDir;

    if (!fs.existsSync(sessionDir)) {
      const err = new Error(`Directory does not exist: ${sessionDir}`);
      err.statusCode = 400;
      logger.error(`Create session failed: missing directory ${sessionDir}`);
      throw err;
    }

    logger.info(`Resolved session directory: ${sessionDir}`);
    ...
    const session = {
      ...
      write(data) {
        ...
        return ptyManager.write(data);
      },
      isRunning() {
        return sessions.has(id) && ptyManager.isRunning();
      },
      kill() {
        if (sessions.has(id)) {
          clearTimeout(session.completionTimer);
  ...
  const scheduleTimer = setInterval(() => {
    ...
    for (const job of jobStore.list()) {
      ...
      jobStore.patchRuntime(job.id, {
        nextRunAt: null,
        lastRunAt: now,
        lastRunStatus: 'queued',
        lastRunMessage: ''
      });
      jobRunner.run(job.id, { manual: false }).catch(error => {
        logger.error(`Scheduled task ${job.id} failed: ${error.message}`);
      });
    }
  }, 30000);
  ...
  app.post('/api/sessions', async (req, res) => {
    ...
    try {
      ...
      const session = createManagedSession({ toolKey, workingDirectory });
