...
async function webCommand(options) {
  ...
  app.get('/api/sessions/:id/git-status', async (req, res) => {
    ...
    const result = await execPromise('git status --porcelain=v2 --branch --untracked-files=all', session.ptyManager.workingDir);
    if (!result.success) {
  ...
  app.get('/api/sessions/:id/git-diff-numstat', async (req, res) => {
    ...
    const result = await execPromise(cmd, session.ptyManager.workingDir);
