...
<body>
    ...
    <script>
        ...
        async function createSession(toolKey) {
            try {
                const res = await fetchWithTimeout('/api/sessions', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ toolKey }) });
        ...
        function performSend() {
            ...
            if (val) { sendWS(val + '\\r'); inputEl.value = ''; inputEl.style.height = '38px'; syncLayout(); }
        }
