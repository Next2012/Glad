...
func TestCodexBlockingQuestionWaitsForExplicitCompleteAnswer(t *testing.T) {
	...
	if err := provider.AnswerUserInput(context.Background(), id, "answer", map[string]string{"count": "1000"}); err == nil || len(writes) != 0 {
		t.Fatal("incomplete answer released the request")
	}
	answers := map[string]string{"count": "1000", "people": "2"}
	if err := provider.AnswerUserInput(context.Background(), id, "answer", answers); err != nil {
		t.Fatal(err)
	}
	...
	if err := provider.AnswerUserInput(context.Background(), id, "answer", answers); err != nil || len(writes) != 0 {
		t.Fatal("retry was not idempotent")
	}
	if err := provider.AnswerUserInput(context.Background(), id, "different-client", answers); err == nil {
...
func TestCodexAsyncQuestionSteersAndPreservesIdleCompletion(t *testing.T) {
	...
	go func() {
		result <- provider.AnswerUserInput(context.Background(), id, "answer", map[string]string{"q1": "1000", "q2": "2"})
	}()
...
func TestCodexAsyncQuestionCanBeAnsweredAfterTurnEnds(t *testing.T) {
	...
	result := make(chan error, 1)
	go func() {
		result <- provider.AnswerUserInput(context.Background(), id, "late-answer", map[string]string{"q1": "1000", "q2": "2"})
	}()
	var request map[string]any
	select {
	case raw := <-writes:
...
func TestCodexQuestionCancellationAndHistory(t *testing.T) {
	for _, reason := range []string{"interrupt", "transport", "resolved"} {
		t.Run(reason, func(t *testing.T) {
			...
			if err := provider.AnswerUserInput(context.Background(), id, "late", map[string]string{"count": "1000", "people": "2"}); err == nil || len(writes) != 0 {
...
func TestCodexSecretAnswerIsOnlySentToProvider(t *testing.T) {
	...
	if err := provider.AnswerUserInput(context.Background(), id, "secret-answer", map[string]string{"secret": "private-value"}); err != nil {
		t.Fatal(err)
	}
