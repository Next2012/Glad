...
% --- SERVER NODE ---
\node[glad_box] (glad) {\textbf{Glad Server}\\[3pt] \footnotesize\color{cText} Node.js Process (Port 3000)};

% Sessions
\node[session_box, below=2cm of glad, xshift=-1.6cm] (s2) {Codex};
\node[session_box, left=0.5cm of s2] (s1) {Claude};
\node[session_box, right=0.5cm of s2] (s3) {Gemini};
\node[session_box, right=0.5cm of s3] (s4) {\dots};
...
\node[server_wrapper, fit=(server_top)(server_bottom)(s1)(s4)] (server) {};
